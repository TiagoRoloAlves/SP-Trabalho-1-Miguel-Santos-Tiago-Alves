...
species_list = {'H2', 'O2', 'N2', 'H2O', 'OH', 'O', 'H'}; % Relevant species for H2 combustion
fuel = 'H2'; % Hydrogen fuel
oxidizer = {'O2', 'N2'}; % Oxidizer
molar_mass_fuel = 2.016; % H2 molar mass in g/mol

% Results storage
adiabatic_flame_temp = zeros(size(phi_range));
product_mole_fractions = zeros(length(phi_range), length(species_list));
combustion_enthalpy_kmol = zeros(size(phi_range)); % Combustion enthalpy per kmol of H2
combustion_enthalpy_kg = zeros(size(phi_range));   % Combustion enthalpy per kg of H2
...
for i = 1:length(phi_range)
    ...
    reactants = sprintf('%s:%f, %s:%f, %s:%f', ...
        fuel, phi, oxidizer_mixture{1}, 0.5/phi, oxidizer_mixture{3}, 0.5/phi*3.76); % Adjust oxidizer for stoichiometry
    ...
    % Combustion enthalpy (standard conditions, per kmol and per kg of H2)
...
title('Adiabatic Flame Temperature vs. Equivalence Ratio (H2)');
grid on;
...
title('Product Mole Fractions vs. Equivalence Ratio (H2)');
...
ylabel('Combustion Enthalpy (J/kmol of H2)');
yyaxis right;
plot(phi_range, combustion_enthalpy_kg, 'r--', 'LineWidth', 1.5);
ylabel('Combustion Enthalpy (J/kg of H2)');
xlabel('Equivalence Ratio (\phi)');
title('Combustion Enthalpy vs. Equivalence Ratio (H2)');
legend({'J/kmol of H2', 'J/kg of H2'}, 'Location', 'best');
grid on;
